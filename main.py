...
def process_image(image, model_id, image_size, conf_threshold, correction, sat_factor, kernel_size, DO, t, vertical):
    ...
    clipped_image = clip_image(blurred_image / 255.0, correction, sat_factor) * 255
    wrapped_image = wrap_image(blurred_image / 255.0, correction, sat_factor) * 255

    original_annotated, original_detections = yolov10_inference(original_image, model_id, image_size, conf_threshold)
    clipped_annotated, clipped_detections = yolov10_inference(clipped_image.astype(np.uint8), model_id, image_size, conf_threshold)
    wrapped_annotated, wrapped_detections = yolov10_inference(wrapped_image.astype(np.uint8), model_id, image_size, conf_threshold)

    img_tensor = transforms.ToTensor()(wrapped_image).unsqueeze(0)
    # Assuming `recons` is a function in `utils.py`
    recon_image = recons(img_tensor, DO=int(DO), L=1.0, vertical=(vertical == "True"), t=t)
    recon_image_pil = transforms.ToPILImage()(recon_image.squeeze(0))
    recon_image_np = np.array(recon_image_pil)
