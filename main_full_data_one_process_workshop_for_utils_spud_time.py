...
def process_image(image_path, annotations_path, model_id, image_size, conf_threshold, correction, sat_factor, kernel_size, DO, t, vertical):
    ...
    #recon_image = recons_spud(img_tensor, DO=1, L=1.0, vertical=(vertical == "True"), t=t)
    ...
    process_time = time.time() - start_time  # Calcular el tiempo total de procesamiento
    return original_annotations, LDR_detections, clipped_detections, wrapped_detections, recon_detections, process_time
...
def process_dataset(dataset_dir, model_ids, image_size, conf_threshold, correction, sat_factors, kernel_size, DO, t, vertical, ranges, process_types):
    ...
    all_times = {(process_type, sat_factor, model_id): [] for process_type in process_types for sat_factor in sat_factors for model_id in model_ids}
            ...
            if any(start <= image_id <= end for start, end in ranges):
                ...
                for model_id in model_ids:
                    for sat_factor in sat_factors:
                        original_annotations, LDR_detections, clipped_detections, wrapped_detections, recon_detections, process_time = process_image(
                        ...
                        for process_type in process_types:
                            ...
                            all_times[(process_type, sat_factor, model_id)].append(process_time)
    ...
    total_process_times = {(process_type, sat_factor, model_id): sum(times) for (process_type, sat_factor, model_id), times in all_times.items()}

    for (process_type, sat_factor, model_id), metrics in global_metrics.items():
        for (process_type, sat_factor, model_id), time in total_process_times.items():
            save_metrics_to_txt(dataset_dir, image_size, conf_threshold, correction, sat_factor, kernel_size, DO, t, vertical, metrics, process_type, ranges, model_id, time)

    return global_metrics, total_process_times

def save_metrics_to_txt(dataset_dir, image_size, conf_threshold, correction, sat_factor, kernel_size, DO, t, vertical, metrics, process_type, ranges, model_id, process_time):
    ...
    with open(metrics_file_path, 'w') as f:
        ...
        for key, value in metrics.items():
            f.write(f"{key}: {value}\n")
        f.write(f"\nTotal Processing Time: {process_time:.2f} seconds\n")
...
if __name__ == "__main__":
    ...
    #model_ids = ["yolov10n", "yolov10m", "yolov10b"]
    model_ids = ["yolov10s"]
    image_size = 640
    conf_threshold = 0.60
    correction = 1
    ...
    #sat_factors = [1.5,2,3,4]
    sat_factors = [1.5]
    #process_types = ['original', 'clip', 'wrap', 'recon']
    process_types = ['clip', 'recon']

    global_metrics, total_process_times = process_dataset(
        dataset_dir, model_ids, image_size, conf_threshold, correction, sat_factors, kernel_size, DO, t, vertical, ranges, process_types
    )

    # Print global metrics and times
    for (process_type, sat_factor, model_id), metrics in global_metrics.items():
        print(f"Global Metrics for {process_type.capitalize()} Images with Saturation {sat_factor} using {model_id}:", metrics)
    for (process_type, sat_factor, model_id), time in total_process_times.items():
        print(f"Total Time for {process_type.capitalize()} Images with Saturation {sat_factor} using {model_id}: {time:.2f} seconds")
